...
--
-- Host: localhost    Database: misige
-- ------------------------------------------------------
...
--
-- Current Database: `misige`
--

CREATE DATABASE /*!32312 IF NOT EXISTS*/ `misige` /*!40100 DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci */;
...
CREATE TABLE `assistentes` (
  ...
  `ano` int(4) NOT NULL,
  PRIMARY KEY (`assistente_id`,`curso_id`,`cadeira_id`,`ano`),
  KEY `fk_assistente_turma_id` (`curso_id`,`cadeira_id`,`ano`),
  CONSTRAINT `fk_assistente_assistente_id` FOREIGN KEY (`assistente_id`) REFERENCES `docentes` (`id`),
  CONSTRAINT `fk_assistente_turma_id` FOREIGN KEY (`curso_id`, `cadeira_id`, `ano`) REFERENCES `turmas` (`curso_id`, `cadeira_id`, `ano`)
...
DROP TABLE IF EXISTS `avaliacao_nota`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `avaliacao_nota` (
  ...
  `peso` float NOT NULL,
  PRIMARY KEY (`curso_id`,`cadeira_id`,`ano`,`estudante_id`,`nome_avaliacao`),
  CONSTRAINT `avaliacao_nota_ibfk_1` FOREIGN KEY (`curso_id`, `cadeira_id`, `ano`, `estudante_id`) REFERENCES `medias` (`curso_id`, `cadeira_id`, `ano`, `estudante_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
LOCK TABLES `avaliacao_nota` WRITE;
/*!40000 ALTER TABLE `avaliacao_nota` DISABLE KEYS */;
INSERT INTO `avaliacao_nota` VALUES
(3,21,2020,73,'teste teste 2',10,0.7),
(3,21,2024,73,'teste trigger criarAvaliacoes',0,0.3),
(3,21,2024,76,'teste trigger criarAvaliacoes',0,0.3),
(3,21,2024,78,'teste trigger criarAvaliacoes',0,0.3),
(3,21,2024,79,'teste trigger criarAvaliacoes',0,0.3),
(3,21,2024,81,'teste trigger criarAvaliacoes',0,0.3),
(3,21,2024,82,'teste trigger criarAvaliacoes',0,0.3),
(3,21,2024,88,'teste trigger criarAvaliacoes',0,0.3),
(3,21,2024,90,'teste trigger criarAvaliacoes',0,0.3),
(3,21,2024,92,'teste trigger criarAvaliacoes',0,0.3),
(3,21,2024,94,'teste trigger criarAvaliacoes',0,0.3);
/*!40000 ALTER TABLE `avaliacao_nota` ENABLE KEYS */;
UNLOCK TABLES;
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER actualizar_media after INSERT ON `avaliacao_nota` FOR EACH ROW
BEGIN

Select media into @media from medias where curso_id = new.curso_id and cadeira_id=new.cadeira_id and ano = new.ano and estudante_id = new.estudante_id;

set @media = @media+new.nota*new.peso;

update medias set media=@media 
where curso_id = new.curso_id and cadeira_id=new.cadeira_id and ano = new.ano and estudante_id = new.estudante_id;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER t_actualizarMedias AFTER INSERT ON avaliacao_nota
FOR EACH ROW
BEGIN
DECLARE row_avaliacaoNota  ROW TYPE OF avaliacao_nota;
DECLARE tempMedia float(5,3) DEFAULT 0;
DECLARE done INT DEFAULT 0;
DECLARE cur_avaliacaoNota CURSOR FOR
Select
  *
from
  avaliacao_nota
Where
  ...
  AND cadeira_id = NEW.cadeira_id AND
  curso_id=NEW.curso_id AND
  ano = NEW.ano;
  DECLARE CONTINUE HANDLER FOR NOT FOUND
SET
  done = 1;
  OPEN cur_avaliacaoNota;
  actualizarMedia:
  LOOP FETCH cur_avaliacaoNota INTO row_avaliacaoNota;
  IF done = 1  THEN LEAVE actualizarMedia;END IF;
  SET @oldMedia = tempMedia;
  
  set tempMedia = @oldMedia + (row_avaliacaoNota.nota*row_avaliacaoNota.peso);
  END LOOP actualizarMedia;
  CLOSE cur_avaliacaoNota;
  UPDATE medias SET media=tempMedia WHERE 
  estudante_id = NEW.estudante_id
  AND cadeira_id = NEW.cadeira_id AND
  curso_id=NEW.curso_id AND
  ano = NEW.ano;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER t_actualizarMediasUpdate AFTER update ON avaliacao_nota
FOR EACH ROW
BEGIN
DECLARE row_avaliacaoNota  ROW TYPE OF avaliacao_nota;
DECLARE tempMedia float(5,3) DEFAULT 0;
...
CREATE TABLE `avaliacoes` (
  ...
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`curso_id`,`cadeira_id`,`ano`,`nome_avaliacao`)
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER t_criarAvaliacao AFTER INSERT ON avaliacoes
FOR EACH ROW
 BEGIN 
DECLARE row_medias ROW TYPE OF medias;
  DECLARE done int DEFAULT 0;
 
DECLARE cur_medias CURSOR FOR
Select
  *
from
  medias
Where
  cadeira_id = NEW.cadeira_id
...
OPEN cur_medias;
  criarAvaliacao: 
 LOOP FETCH cur_medias INTO row_medias;
 IF done = 1 THEN LEAVE criarAvaliacao;END IF;
INSERT INTO
  avaliacao_nota(curso_id, cadeira_id, ano, estudante_id,nome_avaliacao,nota,peso)
values
  (
    row_medias.curso_id, row_medias.cadeira_id,row_medias.ano,row_medias.estudante_id,NEW.nome_avaliacao,0,NEW.peso
  );END LOOP criarAvaliacao; 
  CLOSE 
  cur_medias;
  END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER t_actualizarPeso AFTER UPDATE ON avaliacoes
FOR EACH ROW
 BEGIN 
DECLARE row_medias ROW TYPE OF medias;
  DECLARE done int DEFAULT 0;
...
OPEN cur_medias;
 ...
 LOOP FETCH cur_medias INTO row_medias;
 IF done = 1 THEN LEAVE criarAvaliacao;END IF;
UPDATE avaliacao_nota SET peso=NEW.peso
WHERE 
curso_id=row_medias.curso_iD AND
cadeira_id=row_medias.cadeira_id AND
ano = row_medias.ano AND
estudante_id=row_medias.estudante_id AND 
nome_avaliacao=NEW.nome_avaliacao
;END LOOP criarAvaliacao; 
  CLOSE 
  cur_medias;
  END */;;
...
DROP TABLE IF EXISTS `cadeira_curso`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `cadeira_curso` (
  ...
  KEY `fk_cadeira_curso_curso_id` (`curso_id`),
  CONSTRAINT `fk_cadeira_curso_cadeira_id` FOREIGN KEY (`cadeira_id`) REFERENCES `cadeiras` (`id`),
  CONSTRAINT `fk_cadeira_curso_curso_id` FOREIGN KEY (`curso_id`) REFERENCES `cursos` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
LOCK TABLES `cadeira_curso` WRITE;
/*!40000 ALTER TABLE `cadeira_curso` DISABLE KEYS */;
INSERT INTO `cadeira_curso` VALUES
(21,3,NULL,2024,NULL,'1'),
(22,3,NULL,2024,NULL,'1'),
(23,3,NULL,2024,NULL,'1'),
(24,3,NULL,2024,NULL,'1'),
(25,3,NULL,2024,NULL,'2'),
(26,3,NULL,2024,NULL,'2'),
(27,3,NULL,2024,NULL,'2'),
(28,3,NULL,2024,NULL,'2'),
(29,3,NULL,2024,NULL,'2'),
(30,3,NULL,2024,NULL,'2');
/*!40000 ALTER TABLE `cadeira_curso` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `cadeiras` (
  ...
  PRIMARY KEY (`id`),
  KEY `fk_cadeira_departamento` (`departamento_id`),
  CONSTRAINT `fk_cadeira_departamento` FOREIGN KEY (`departamento_id`) REFERENCES `departamentos` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci COMMENT='Lista de cadeiras presentes no sistema';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `cursos` (
  ...
  PRIMARY KEY (`id`),
  KEY `fk_curos_departamento_id` (`departamento_id`),
  CONSTRAINT `fk_curos_departamento_id` FOREIGN KEY (`departamento_id`) REFERENCES `departamentos` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION,
  CONSTRAINT `fk_curso_departamento_id` FOREIGN KEY (`departamento_id`) REFERENCES `departamentos` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci COMMENT='Cursos que a universidade oferece';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `docentes` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_docente_id` FOREIGN KEY (`id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `estudantes` (
  ...
  `created_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uc_numero_estudante` (`numero_estudante`),
  KEY `fk_estudante_curso_id` (`curso_id`),
  CONSTRAINT `fk_estudante_curso_id` FOREIGN KEY (`curso_id`) REFERENCES `cursos` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE,
  CONSTRAINT `fk_student_id` FOREIGN KEY (`id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=95 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Subtabela de users com info dos estudantes';
/*!40101 SET character_set_client = @saved_cs_client */;
...
(76,20202102,3,'2024-10-17','2030-10-17',NULL,NULL),
(77,20202103,3,'2024-10-17','2030-10-17',NULL,NULL),
(78,20202105,3,'2024-10-17','2030-10-17',NULL,NULL),
(79,20202106,3,'2024-10-17','2030-10-17',NULL,NULL),
(81,20202107,3,'2024-10-17','2030-10-17',NULL,NULL),
...
CREATE TABLE `medias` (
  ...
  PRIMARY KEY (`curso_id`,`cadeira_id`,`ano`,`estudante_id`),
  KEY `fk_media_estudante` (`estudante_id`),
  CONSTRAINT `fk_media_estudante` FOREIGN KEY (`estudante_id`) REFERENCES `estudantes` (`id`),
  CONSTRAINT `fk_media_turma` FOREIGN KEY (`curso_id`, `cadeira_id`, `ano`) REFERENCES `turmas` (`curso_id`, `cadeira_id`, `ano`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `papeis` (
  ...
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Lista de posições ou roles que podem ser assumidos no sistema';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `papel_permissao` (
  ...
  KEY `papel_permissao_permisaoid_foreign` (`permissao_id`),
  CONSTRAINT `papel_permissao_papelid_foreign` FOREIGN KEY (`papel_id`) REFERENCES `papeis` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `papel_permissao_permisaoid_foreign` FOREIGN KEY (`permissao_id`) REFERENCES `permissoes` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `papel_permissao` VALUES
...
('4QdKJbOunB2xihZXMU4vLGLtreiP2poF3AYoZZIh',NULL,'127.0.0.1','PostmanRuntime/7.42.0','YTozOntzOjY6Il90b2tlbiI7czo0MDoiYmFiMFVuUUZXejRDNEN5bmU4cUVqMEgxZ3Rzb0VEV3BvcVN0T3hidCI7czo5OiJfcHJldmlvdXMiO2E6MTp7czozOiJ1cmwiO3M6MjE6Imh0dHA6Ly8xMjcuMC4wLjE6ODAwMSI7fXM6NjoiX2ZsYXNoIjthOjI6e3M6Mzoib2xkIjthOjA6e31zOjM6Im5ldyI7YTowOnt9fX0=',1729012313),
...
('TCv8f0SZlfXSwuerVe86yJ9YclI7Fn13V1NESyge',NULL,'127.0.0.1','PostmanRuntime/7.42.0','YTozOntzOjY6Il90b2tlbiI7czo0MDoiN1V0ZHRiUDI0dTNyWGVETk9iaDdCckVUQUxrYjlEVmxuSnRyeUlOZiI7czo5OiJfcHJldmlvdXMiO2E6MTp7czozOiJ1cmwiO3M6MjE6Imh0dHA6Ly8xMjcuMC4wLjE6ODAwMCI7fXM6NjoiX2ZsYXNoIjthOjI6e3M6Mzoib2xkIjthOjA6e31zOjM6Im5ldyI7YTowOnt9fX0=',1729151540);
...
CREATE TABLE `turmas` (
  ...
  PRIMARY KEY (`curso_id`,`cadeira_id`,`ano`),
  KEY `fk_turmas_cadeira_curso` (`cadeira_id`,`curso_id`),
  KEY `fk_turmas_regente` (`regente_id`),
  CONSTRAINT `fk_turma_cadeira_id` FOREIGN KEY (`cadeira_id`) REFERENCES `cadeiras` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE,
  CONSTRAINT `fk_turma_curso_id` FOREIGN KEY (`curso_id`) REFERENCES `cursos` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE,
  CONSTRAINT `fk_turmas_cadeira_curso` FOREIGN KEY (`cadeira_id`, `curso_id`) REFERENCES `cadeira_curso` (`cadeira_id`, `curso_id`),
  CONSTRAINT `fk_turmas_curso_id` FOREIGN KEY (`curso_id`) REFERENCES `cursos` (`id`),
  CONSTRAINT `fk_turmas_regente` FOREIGN KEY (`regente_id`) REFERENCES `docentes` (`id`),
  CONSTRAINT `fk_turmas_regente_id` FOREIGN KEY (`regente_id`) REFERENCES `docentes` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `users` (
  ...
  KEY `users_papelid` (`papel_id`),
  CONSTRAINT `users_papelid` FOREIGN KEY (`papel_id`) REFERENCES `papeis` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=133 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Tabela geral de usuários';
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2024-10-19 13:35:11
